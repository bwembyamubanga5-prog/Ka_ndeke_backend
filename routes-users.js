...
// safe atomic balance change helper
async function changeBalanceAtomic(db, userId, delta) {
  try {
    ...
    await db.run("COMMIT");
    const user = await db.get("SELECT id, username, phone, balance, freeRounds, createdAt, updatedAt FROM users WHERE id = ?", [userId]);
    return user;
  } catch (err) {
...
// body: { username, phone, password }  -- phone is the required identifier now
router.post("/auth/register", async (req, res) => {
  try {
    const { username, phone, password } = req.body;
    if (!username || !phone || !password) return res.status(400).json({ error: "Missing fields (username, phone, password required)" });
    ...
    await db.run(
      `INSERT INTO users (id, username, phone, password_hash, balance, freeRounds, createdAt, updatedAt)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [id, username, phone, hash, 0, 0, now, now]
    );

    const user = await db.get("SELECT id, username, phone, balance, freeRounds FROM users WHERE id = ?", [id]);
...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    if (!phone || !password) return res.status(400).json({ error: "Missing fields (phone, password required)" });

    const db = getDb(req);
    const row = await db.get("SELECT id, password_hash FROM users WHERE phone = ?", [phone]);
    if (!row) return res.status(401).json({ error: "Invalid credentials" });

    const ok = await bcrypt.compare(password, row.password_hash);
    if (!ok) return res.status(401).json({ error: "Invalid credentials" });

    const user = await db.get("SELECT id, username, phone, balance, freeRounds FROM users WHERE id = ?", [row.id]);
...
router.get("/users/me", authMiddleware, async (req, res) => {
  try {
    const db = getDb(req);
    const user = await db.get("SELECT id, username, phone, balance, freeRounds, createdAt, updatedAt FROM users WHERE id = ?", [req.user.id]);
